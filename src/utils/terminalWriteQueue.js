...
// 带内全量重置（替代带外 terminal.reset()）：reset() API 立即清解析器状态，但 xterm 内部
// WriteBuffer 里已 write 未解析的字节会在 reset **之后**继续解析——它们的起点是解析暂停点
// （可能在序列正中间），ground state 下半截序列按字面渲染成 `0;134m` 类残片并永久留在新
// scrollback 里。改为把重置作为字节流推进写队列，按序排在残留内容之后，解析器状态全程连续：
//   \x07 BEL —— 若解析器停在 OSC payload 中，终结之（ground 态下是无声 bell，无害）
//   \x18 CAN —— 若停在 CSI/ESC 中，中止之（BEL 在 CSI 态只执行不打断，故需补 CAN）
//   \x1bc RIS —— 全量硬重置（xterm 实现为 Terminal.reset() 等价：清屏 + scrollback + 属性）
export const INBAND_RESET = '\x07\x18\x1bc';
